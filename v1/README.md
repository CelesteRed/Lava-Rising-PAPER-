...
1. Put `LavaRising-1.2.36-paper26.1.2-set-lobby-location.jar` in your server `plugins` folder.
...
When no round is active, players are kept in the lobby at the configured lobby X/Z. The plugin loads the lobby chunk, moves joiners there after spawn handling finishes, and periodically returns waiting players who leave the lobby radius. A round can start when at least `minPlayersToStart` players are in the lobby. If no OP/admin is online, any player with `lavarising.start` can use `/start`. If an OP/admin is online, an OP/admin must start it.
...
Revive an eliminated player during a running round:

```text
/revive <player> [targetPlayer]
```

If `targetPlayer` is blank, the player revives onto a random alive player. Only OP players can use `/revive`.
...
You can also set the center directly:

```text
/lavarising settings lobbyX 0.5
/lavarising settings lobbyZ 0.5
```
...
How many seconds the winner celebration lasts before everyone returns to lobby. During the celebration, fireworks spawn around the winner every `0.5` seconds.

```text
/lavarising settings celebrationTime 15
```
...
How many building blocks are given per lava rise.
...
Maximum number of the round building block each player can hold from the plugin's block giver.

```text
/lavarising settings maxGivenBlocks 256
```
...
How far players can move from the lobby center while waiting for a round.

```text
/lavarising settings lobbyRadius 32
```
...
Legacy setting. Current behavior keeps PVP off during lobby/countdown, then enables PVP when the live round starts.
...
Controls extra server-console logs for starts, arena selection, lobby teleports, deaths, lava milestones, and lava integrity repairs.

```text
/lavarising settings consoleLogging true
```
...
- Lobby players are kept in adventure mode.
- Waiting players cannot leave the configured `lobbyRadius`; leaving it returns them to the lobby center.
- OP/admin players bypass the waiting lobby boundary so they can set or inspect the lobby location.
...
- Round players spawn around the selected arena center in a circle in adventure mode before countdown.
- Round countdown starts the world difficulty in `EASY`.
- Lobby and arena spawn locations must be safe top-surface locations at or above `Y=60`; cave/underground floors are rejected.
- Lobby and countdown keep PVP off for a clean round start.
- Countdown finish puts active round players into survival mode and enables PVP for the live round.
- Active round players can damage other active players and living entities once the countdown finishes.
- Eliminated round players, including OPs, are moved to spectator mode after they respawn from the death screen.
- OPs can revive a player during a running round with `/revive <player> [targetPlayer]`.
- The winner gets a configurable celebration before the lobby return, with 1-8 fireworks around them every `0.5` seconds.
- During the winner celebration, online players are put in adventure mode, made visible, and can fly until `celebrationTime` finishes.
- After the celebration lobby return, everyone is put in adventure mode with flight disabled, player inventories and potion effects are cleared, and all non-player entities are removed from the overworld.
- OPs are not forced into adventure or survival by automatic game flow, but OPs are still eliminated into spectator when they die in a round.
- Extra console logs are enabled by default with `consoleLogging: true`.
- Lava rises to `Y=319`.
- Air, water, leaves, wood, doors, and trapdoors below the current lava height are repaired into lava in active arena chunks.
- Phase 5 starts at `Y=314`.
- Sudden death starts instantly when phase 5 starts at `Y=314`, and lava keeps rising through phase 5 until `Y=319`.
- The action bar shows lava height, PVP ON/OFF, and lava speed instead of phase text.
